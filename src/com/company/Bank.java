...
public class Bank {
    ...
    public boolean addCustomerTransation(String branchName, String customerName, double amount) {
        Branch branch = findBranch(branchName);
        if (branch != null) {
    ...
    public Branch findBranch(String branchName) {
        for (int i = 0; i < this.branches.size(); i++) {
            Branch chechedBranch = this.branches.get(i);
            if (chechedBranch.getName().equals(branchName)) {
                return chechedBranch;
            }
        }
        return null;
    }
